...
# exclusive_terms_100 %>%
#   arrange(positive_to_negative_ratio) %>%
#   slice(1:10) %>%
#   ggplot(aes(x = term, y = proportion_of_party, color = party)) + 
#   geom_point() +
#   facet_wrap(~party)


# x <- exclusive_terms_100 %>%
#   filter(party =="R") %>%
#   arrange(desc(proportion_of_party)) %>%
#   slice(1:7) %>%
#   ggplot(aes(x = term, y = proportion_of_party, color = party)) + 
#   geom_point() + theme_bw()
# 
# 
# exclusive_terms_100_D <-
#   exclusive_terms_100 %>%
#   filter(party == "D") %>%
#   arrange(desc(proportion_of_party)) %>%
#   slice(1:7) %>%
...
blm_votes <- left_join(blm_google, votes_clean, by = "month_year") %>%
...
metoo_votes <- left_join(metoo_google, votes_clean, by = "month_year") %>%
  #mutate(month_year = format(as.Date(month_year), "%Y-%m")) %>%
  #mutate(month_year = as.numeric(month_year)) %>%
...
#   geom_line(aes(y = police_bill_prop, linetype="a")) +
#   geom_line(aes(y = mean_score, linetype="b")) +
#   scale_y_continuous(sec.axis = sec_axis(~., name = "BLM"))
...
#METOO GRAPHS
 ...
 ## this is by week and we probably want this by month, but maybe it would be good to have by week??
 ## I think we can take the mean weeks within the total of the month to not lose any data


  #votes$Month_Yr <- format(as.Date(votes$status_date), "%Y-%m")
  # groupby ... prop of bills that were in that month 
# for each year, prop of bills that are about harassment/etc. 
# get multiple years and filter for introduced 
```
